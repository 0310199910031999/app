        ...
        try:
            ...
            if dto.status == "Cerrado" and model.client_id:
                if model.client and model.client.email and model.equipment:
                    ...
                    # Construir subject
                    subject = f"FO-EM-01 {brand_name} #{economic_number} {file_id}"
                    
                    # Construir message con enlace al reporte
                    from config import settings
                    report_url = f"{settings.BASE_URL}/foem01/{model.id}/reporte"
                    message = f"""
                    <html>
                    ...
                    def send_email_async():
                        try:
                            ...
                            print(f"[FOEM01] Advertencia: No se pudo enviar email: {str(e)}")
                    ...
                    try:
                        ...
                        print(f"[FOEM01] Advertencia: No se pudo encolar notificación push: {str(e)}")
